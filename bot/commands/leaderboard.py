...
def setup(bot: TajinHelper) -> None:
    ...
    async def vagudle_duel_leaderboard(
        interaction: discord.Interaction,
        user: Optional[discord.User] = None,
    ):
        logger.info(
            f"/vagudle_duel_leaderboard called by {interaction.user} (id={interaction.user.id}) "
    ...
    @bot.tree.command(
        name="vagudle_daily_leaderboard",
        description="View the Vagudle Daily leaderboard",
    )
    @app_commands.allowed_installs(guilds=True, users=True)
    @app_commands.allowed_contexts(guilds=True, dms=True, private_channels=True)
    @app_commands.describe(
        user="Look up a specific player's rank and stats (Discord-linked accounts only)"
    )
    async def vagudle_daily_leaderboard(
        interaction: discord.Interaction,
        user: Optional[discord.User] = None,
    ):
        logger.info(
            f"/vagudle_daily_leaderboard called by {interaction.user} (id={interaction.user.id}) "
            f"— lookup={'@' + str(user) if user else 'none'}"
        )
        await interaction.response.defer()
        ...
        embed, total_pages = await build_daily_leaderboard_embed(
            bot, all_rows, 1, user
        )
