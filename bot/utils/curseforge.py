        ...
        async with async_playwright() as p:
            ...
            try:
                ...
                if not response or response.status != 200:
                    logger.warning(
                        f"[CurseForge scraper] page returned HTTP {response.status if response else 'no response'}"
                    )
                    return None
                ...
                match = re.search(
                    r"([\d,]+\.?\d*[KkMmBb]?)\s+Followers?", text_content, re.IGNORECASE
                )
                if match:
                    count = parse_abbreviated_number(match.group(1))
                    logger.debug(
                        f"[CurseForge scraper] found followers: {match.group(1)} → {count}"
                    )
                    return count
    ...
    try:
        ...
        while True:
            ...
            async with session.get(
                ...
                if resp.status != 200:
                    logger.warning(
                        f"[CurseForge API] search returned {resp.status} on page {page}"
                    )
                    return None
        ...
        logger.debug(
            f"[CurseForge API] fetched {project_count} projects across {page + 1} page(s), total downloads: {total_downloads:,}"
        )
        ...
        if project_count in range(current_year - 1, current_year + 2):
            logger.warning(
                f"[CurseForge API] project count ({project_count}) looks like a year, setting to 0"
            )
            project_count = 0
        ...
        logger.error(
            f"[CurseForge API] unexpected error: {e}\n{traceback.format_exc()}"
        )
        return None
...
async def get_curseforge_stats(username: str) -> dict | None:
    ...
    logger.info(
        f"[CurseForge] downloads={api_result['total_downloads'] if api_result else 'N/A'} projects={api_result['project_count'] if api_result else 'N/A'} followers={followers}"
    )
