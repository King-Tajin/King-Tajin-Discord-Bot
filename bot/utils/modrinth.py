    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(
                f"{base_url}/user/{username}", timeout=aiohttp.ClientTimeout(total=30)
            ) as response:
            ...
            async with session.get(
                f"{base_url}/user/{user_id}/projects",
                timeout=aiohttp.ClientTimeout(total=30),
            ) as response:
                if response.status != 200:
                    logger.error(
                        f"Modrinth projects API returned status {response.status}"
                    )
                    return None
